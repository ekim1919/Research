 ...
 \Fun{\TransformBund{$f$, $Q$, $\Lambda^Q$}}{
   ...
   \For{$P \in \ptopeset{Q}$}{
     ...
     \For{each $\Lambda_i^Q$ in $\Lambda^Q$ }{
       $c_{u}'[i] \gets \mathsf{min}\{ \mathsf{optBox(G_P)}, c_{u}'[i] \}$ (Equation~\ref{eq:maxalpha})\; \label{ln:bern_opt_max}
       $c_{l}'[i] \gets \mathsf{max}\{ -1 \times \mathsf{optBox(-1 \times G_P)}, c_{l}'[i] \}$\; \label{ln:bern_opt_min}
     }
   }
